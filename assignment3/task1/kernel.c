...
kernel void reduce(global unsigned long *workpack_result_low,
                     ...
                     local unsigned long *tmp_high)
{
    ...
    while (half_block_size > 0)
    {
        ...
        if (lid < half_block_size)
        {
            ...
            tmp_high[lid] += tmp_high[lid + half_block_size] 
                    + (tmp_low[lid] < tmp_low[lid + half_block_size]);
            //if uneven
            if ((half_block_size * 2) < group_size)
            {
                if (lid == 0)
                {
                    ...
                    tmp_high[0] += tmp_high[group_size - 1] +
                            + (tmp_low[0] < tmp_low[group_size - 1]);
                }
            }
        }
    ...
    if (lid == 0)
    {
        workpack_result_low[group_id] =  tmp_low[0];
        workpack_result_high[group_id] = tmp_high[0];
    }
}