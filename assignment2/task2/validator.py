...
class OptionalFileHashTest(Test):
	...
	def run(self):
		self.exists = True
		try:
			content = open(in_cwd(self.filename), "rb").read()
			h = hashlib.sha256()
			h.update(content)
	...
	def okay(self):
		return not self.exists or self.hash == self.expected

	def what(self):
		if not self.exists:
			return "%s not found, not testing" % self.filename
		return "SHA-256(%s):\n%s%s" % (self.filename, self.hash,
				("" if self.okay() else ("\nexpected:\n%s" % self.expected)))

def CryptTest(passwords, dictionary, expected):

	return AbortingTestGroup(
			EmptyTestLine(),
			CreateFileJob("pwd.txt", passwords),
			CreateFileJob("dict.txt", dictionary),
			ExecutionTest("./decrypt", ["pwd.txt", "dict.txt"], expected, key=sorted),
		)
...
def main(cd=None):
	tests = TestGroup(
		AbortingTestGroup(
			...
			CryptTest(
					"projekt:fg0T45RBV260w\n"
					"flugzeug:rmhwq6sHMswEY\n"
					"wassehen:Ph9qTIiUFzTzA\n"
					"rechner5:zhuHfRYGvjMfs\n",
					"Abbild\n"
					"Abakus\n"
					"Abbitte\n"
					"Abbrecher\n"
					"Fliegen\n",
					["rechner5;Abakus5", "flugzeug;Fliegen"]
				),
			CryptTest(
					"barbera:Gozsjkgq.2N62\n"
					"gene:SqJwiPjc8z9OQ\n"
					"grace:L3xIP64G5RVk6\n"
					"ian:MyIR7zQEkP3Mg\n"
					"sheelagh:7CYgbT6A0xsM6\n"
					"dennis:oGlayhJ1bTXuE\n"
					"margarete:qRbG.QWxv9c.6\n"
					"elon:UFy0LW2XSNPVo\n"
					"satoshi:Hqw9N3HL38lAw\n",
					"accelerator\n"
					"access\n"
					"algorithm\n"
					"application\n"
					...
					"transistor\n"
					"workload\n",
					["ian;partitioning", "gene;speedup0"]
				),
		),
		EmptyTestLine(),
		OptionalFileHashTest("taskCryptSolution.txt", 
		                     "bf6ce532a3c3264f4e76656f48c7e1601e181ab1a772b8991cc074d46ca2f117")
	)
